...
Cubic::Cubic(std::pair<double, double> p0, std::pair<double, double> vertex, std::pair<double, double> vertex2){
    double x0 = p0.first;
    double y0 = p0.second;
    ...
    if (((x0*x0*xv1)+(xv0*xv0*x0)+(xv1*xv1*xv0)) == ((xv0*xv0*xv1)+(x0*x0*xv0)+(xv1*xv1*x0))) {
        throw std::logic_error(
                "pairs passed to Cubic constructor must not have the same first values");
    }
    double m = ((x0*x0*xv1)+(xv0*xv0*x0)+(xv1*xv1*xv0)) - ((xv0*xv0*xv1)+(x0*x0*xv0)+(xv1*xv1*x0));
